    ...
    def run(self):
        ...
        if on_batch_node() and submitted not in (None, family):
            raise RuntimeError(
                ...
                f"{submitted} job must not generate a sample on its own slot. Produce that "
                "seed first, or check that fs_default is readable from the worker, which is the "
...
class NanoMergeTask(Task, HTCondorWorkflow, CrabWorkflow, law.LocalWorkflow):
    ...
    def workflow_requires(self):
        # The whole generation stage, deliberately: merging waits for `RunProd` to finish.
        #
        # Merging each group as its own 50 seeds land is what 169 of the 192 complete-but-unmerged
        # Run3_2023BPix groups asked for, and law cannot express it. `workflow_requires()` IS the
        # proxy's `requires()`, so any edge here blocks the whole merge; a per-seed
        # `req(branch=n)` is a *branch* task, which luigi runs in-process rather than on the
        # batch system; and a per-group `req_different_branching(branches=...)` is a narrowed
        # *workflow*, so every group would carry its own submission -- 192 CRAB tasks for one era.
        # The fix belongs in law (chunked dependencies for remote submission), not in a workaround
        # here, so this waits.
        #
        # `req_different_branching` and not `req` because law copies `branches` through `req()`:
